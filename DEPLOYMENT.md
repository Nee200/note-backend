...
Für die erste Liveschaltung dieses Shops wird ausschließlich das Konto `admin` mit vollständigem Admin-Zugriff eingerichtet. Der Nutzer hat die Einrichtung des Betreiber-Kontos ausdrücklich auf später verschoben. Die Beispieldatei enthält entsprechend einen einzelnen Eintrag in `ADMIN_ACCOUNTS_JSON`. Ein zusätzliches Konto `betreiber` kann später mit eigenem bcrypt-Passworthash und eigenem bestätigtem TOTP-Secret hinzugefügt werden. Eine Kontoverwaltung in der Admin-Oberfläche ist noch nicht implementiert; zusätzliche Konten werden derzeit über die Hosting-Konfiguration verwaltet.

Die Konfiguration weist wiederverwendete TOTP-Secrets und doppelte Benutzernamen ab. Eine Abmeldung betrifft nur die betreffende Sitzung; die Rotation der Zugangsdaten eines Kontos verändert den Konfigurationsfingerabdruck eines anderen Kontos nicht. Jede Person muss ihren eigenen Authenticator vor Aktivierung ihres Kontos eingerichtet haben; anschließend die tatsächliche Anmeldung prüfen. Die Beispielwerte sind keine eingerichteten Zugänge.
